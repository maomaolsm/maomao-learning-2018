...
import com.maomao.springaop.framework.context.support.MaoBeanDefinitionReader;
 ...
 */
public class MaoApplicationContext extends MaoDefaultListableBeanFactory implements MaoBeanFactory {

    private String[] configLocations;

    private MaoBeanDefinitionReader reader;

    // 用来保证注册式单例的容器
    private Map<String, Object> beanCacheMap = new HashMap<String, Object>();

    // 用来存储所有被代理过的对象
    private Map<String, MaoBeanWrapper> beanWrapperMap = new ConcurrentHashMap<String, MaoBeanWrapper>();
    ...
    public void refresh() {
        ...
        // 定位
        this.reader = new MaoBeanDefinitionReader(configLocations);
...
//        demoAction.query(null, null, "test maomao");
    ...
    private void doAutowired() {
        ...
        for (Map.Entry<String, MaoBeanDefinition> beanDefinitionEntry : this.beanDefinitionMap.entrySet()) {
        ...
        for (Map.Entry<String, MaoBeanWrapper> beanWrapperEntry : this.beanWrapperMap.entrySet()) {
    ...
    public void populateBean(String beanName, Object instance) {
        ...
        if (!(clazz.isAnnotationPresent(MaoController.class)
            || clazz.isAnnotationPresent(MaoService.class))) {
            return;
        }
        ...
        for (Field field : fields) {
            ...
            if (!field.isAnnotationPresent(MaoAutowired.class)) {
                continue;
            }
            MaoAutowired maoAutowired = field.getAnnotation(MaoAutowired.class);
            String autowiredBeanName = maoAutowired.value().trim();
    ...
    private void doRegistry(List<String> beanDefinitions) {
        ...
        try {
            for (String className : beanDefinitions) {
                ...
                MaoBeanDefinition maoBeanDefinition = reader.registerBean(className);
                if (maoBeanDefinition != null) {
                    this.beanDefinitionMap.put(
                        maoBeanDefinition.getFactoryBeanName(), maoBeanDefinition
                    );
                }
                ...
                for (Class<?> clazz : interfaces) {
                    ...
                    // 怎么办？这个时候，可以自定义名字
                    this.beanDefinitionMap.put(clazz.getName(), maoBeanDefinition);
                }
                // 到这里为止，容器初始化完毕
            }
    ...
    public Object getBean(String beanName) {
        ...
        try {
            ...
            // 在实例初始化以后调用一次
            maoBeanPostProcessor.postProcessAfterInitialization(instance, beanName);
...
//            populateBean(beanName, instance);
    ...
    private Object instantiationBean(MaoBeanDefinition maoBeanDefinition) {
