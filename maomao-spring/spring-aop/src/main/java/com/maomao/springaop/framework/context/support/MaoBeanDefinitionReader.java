package com.maomao.springaop.framework.context.support;
 ...
 */
public class MaoBeanDefinitionReader {
    ...
    // 定位
    public MaoBeanDefinitionReader(String... locations) {
    ...
    // 只是为了对配置信息进行一个包装
    public MaoBeanDefinition registerBean(String className) {
        if (this.registerBeanClasses.contains(className)) {
            MaoBeanDefinition maoBeanDefinition = new MaoBeanDefinition();
            maoBeanDefinition.setBeanClassName(className);
            maoBeanDefinition.setFactoryBeanName(
                lowerFirstCase(className.substring(className.lastIndexOf(".") + 1))
            );
            return maoBeanDefinition;
        }
