 ...
 */
public class MaoHandlerAdapter {

    private Map<String, Integer> paramMapping;

    public MaoHandlerAdapter(Map<String, Integer> paramMapping) {
     ...
     * @param req
     * @param resp
     * @param maoHandlerMapping 为什么要把 handler 传进来
     *                          因为 handle 中包含了 controller、method、url 信息
     * @return
     */
    public MaoModelAndView handle(HttpServletRequest req, HttpServletResponse resp,
                                  MaoHandlerMapping maoHandlerMapping) throws Exception {
        ...
        Class<?>[] paramTypes = maoHandlerMapping.getMethod().getParameterTypes();
        ...
        Object result = maoHandlerMapping.getMethod()
            .invoke(maoHandlerMapping.getController(), paramValues);

        if (result == null) {
            return null;
        }

        boolean isModelAndView = maoHandlerMapping.getMethod().getReturnType() == MaoModelAndView.class;
        if (isModelAndView) {
            return (MaoModelAndView) result;
        } else {
            return null;
        }
    }
